...
function parsePin(expr: SExpr[]): Pin | null {
  ...
  const name = nameExpr && nameExpr.length > 1 && typeof nameExpr[1] === 'string' ? nameExpr[1] : ''

  const numberExpr = get(expr, 'number')
  const number = numberExpr && numberExpr.length > 1 ? String(numberExpr[1]) : ''
...
function parseComponent(expr: SExpr[], libSymbols: Map<string, LibSymbol>): Component | null {
  ...
  const libName =
    libNameExpr && libNameExpr.length > 1 && typeof libNameExpr[1] === 'string'
      ? libNameExpr[1]
      : null
